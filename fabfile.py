...
env.hosts = ['quanticmindtools.com']
env.user   = "fabric"

git_branch = "master"
releases_dir = "/var/www/utiltools/releases"
git_repo = "/insidevault2015/utiltools.git"
git_user = ""
repo_dir = "/var/www/utiltools/repo"
persist_dir = "/var/www/utiltools/persist"
next_release = "%(time).0f" % {'time': time()}
current_release = "/var/www/utiltools/current"
last_release_file = "/var/www/utiltools/LAST_RELEASE"
current_release_file = "/var/www/utiltools/CURRENT_RELEASE"
...
def init(git_user):
  ...
  if not exists(repo_dir):
    git_repo = get_git_repo(git_user)
  ...
  if not exists("%s/storage" % persist_dir):
      ...
      run("chmod 777 -R %s/storage" % persist_dir)

  if not exists("%s/public/storage" % persist_dir):
      run("mkdir -p %s/public/storage" % persist_dir)
      run("chmod 777 -R %s/public/storage" % persist_dir)
